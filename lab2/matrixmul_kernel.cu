...
__global__ void MatrixMulKernel(Matrix M, Matrix N, Matrix P)
{
    ...
    for (int k = 0; k < M.width/TILE_WIDTH + 1; ++k) {
        // every thread within a block load [TILE_SIZE] into shared memery
        if (k*TILE_WIDTH + threadIdx.x < M.width && row < M.height) {
            Ms[threadIdx.y][threadIdx.x] = M.elements[row*M.width + k*TILE_WIDTH + threadIdx.x];
        }
        else {
            Ms[threadIdx.y][threadIdx.x] = 0.0;
        }

        if (k*TILE_WIDTH + threadIdx.y < N.height && col < N.width) {
            Ns[threadIdx.y][threadIdx.x] = N.elements[(k*TILE_WIDTH + threadIdx.y)*N.width + col];
        } else {
